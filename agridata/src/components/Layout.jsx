...
export default function Layout() {
  ...
  const markAsRead = async (id) => {
    ...
    try {
      ...
      // Revert if failed (optional, usually not needed for simple toggles)
    }
  };
  ...
  const clearNotifications = async () => {
    if (window.confirm("Purge all notification records?")) {
      ...
      try {
        ...
        // If fail, re-fetch to show them again
        fetchNotifications();
      }
    }
  };
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-[#020c0a] overflow-hidden font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
