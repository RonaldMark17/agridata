          ...
          <div className="flex flex-col h-full w-full">
            ...
            <div className="p-3 sm:p-4 mt-auto border-t border-slate-100 dark:border-white/5">
              <div className={`flex items-center rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/5 transition-all duration-500 ${(isCollapsed && !sidebarOpen) ? 'justify-center p-2' : 'p-3 gap-3'}`}>
                <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-emerald-600 to-emerald-400 flex items-center justify-center font-bold text-white shadow-lg shrink-0">
                ...
                {showNotifications && (
                  <div className="fixed top-16 left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 sm:w-[380px] mt-2 sm:mt-4 max-w-[400px] mx-auto bg-white dark:bg-[#0b241f] rounded-3xl sm:rounded-[2rem] shadow-2xl border border-slate-100 dark:border-white/5 overflow-hidden z-[100] animate-in fade-in zoom-in-95">
                    <div className="p-4 sm:p-5 border-b dark:border-white/5 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
                      <h3 className="font-bold text-xs sm:text-sm dark:text-white uppercase tracking-tight">Recent Alerts</h3>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#0b241f] rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl w-[95vw] sm:w-full max-w-lg overflow-hidden animate-in zoom-in-95 border dark:border-white/10">
            <div className="p-5 sm:p-8 border-b border-slate-50 dark:border-white/5 flex items-center justify-between">
              <div>
                <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Preferences</h3>
                <p className="text-slate-400 text-[9px] sm:text-[10px] font-black uppercase tracking-widest mt-1 sm:mt-2">Personalize environment</p>
              </div>
            ...
            <div className="p-5 sm:p-8 space-y-8 sm:space-y-10">
              ...
              {/* DISPLAY MODE */}
              <section>
                <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-emerald-600 mb-4 flex items-center gap-2"><Activity size={14} /> Display Mode</h4>
              ...
              <section className="pt-6 border-t border-slate-100 dark:border-white/5">
                ...
                <div className="flex flex-row items-center justify-between p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] gap-3">
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                    <div className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl transition-colors shrink-0 ${otpEnabled ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200 dark:shadow-none' : 'bg-slate-200 dark:bg-white/10 text-slate-400'}`}>
                      <Smartphone size={18} className="sm:w-[20px] sm:h-[20px]" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">Two-Factor Auth</p>
                      <p className="text-[9px] sm:text-[10px] font-medium text-slate-400 truncate">Email verification</p>
                      {user?.role === 'admin' && otpEnabled && (
                        <p className="text-[8px] sm:text-[9px] font-bold text-rose-500 mt-0.5 uppercase tracking-widest truncate">Req. for Admins</p>
                      )}
                    </div>
                  </div>
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#0b241f] rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl p-6 sm:p-8 max-w-[90vw] sm:max-w-sm w-full text-center border dark:border-white/10 animate-in zoom-in-95">
            ...
            <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tight mb-2 dark:text-white">End Session?</h2>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mb-6 sm:mb-8">Access keys will be cleared. Verification required to resume protocol.</p>
            <div className="flex flex-col gap-2 sm:gap-3">
              <button onClick={handleLogout} className="w-full py-3 sm:py-4 bg-slate-900 dark:bg-emerald-500 text-white dark:text-[#041d18] rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-[0.2em] shadow-xl">Confirm Termination</button>
